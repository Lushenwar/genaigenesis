...
const layers: Layer[] = [
  { id: "vegetation", name: "Vegetation Cover", icon: Trees },
  { id: "population", name: "Population Density", icon: Users },
  { id: "heat", name: "Heat Island Index", icon: Thermometer },
  { id: "financial", name: "Financial Zones", icon: DollarSign },
];
    ...
    <div className="h-full min-h-0 flex flex-col border-r border-border bg-background">
        ...
        {layers.map((layer) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors hover:bg-secondary ${
              activeLayers[layer.id] ? "bg-secondary/60" : ""
            }`}
          >
      ...
      <div className="p-4 border-t border-border space-y-2">
        <div className="flex justify-between text-xs">
          ...
          <span className="font-mono text-foreground font-medium text-foreground">14,202 ha</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-muted-foreground">Risk zones</span>
          <span className="font-mono text-foreground font-medium text-foreground">10</span>
        </div>
      </div>
    </div>
  );
